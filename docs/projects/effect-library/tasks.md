...
## Milestones
- [ ] Milestone 1 — Inventory and design the shared effect system. Acceptance: repo has a reviewed proposal for folder layout, block taxonomy, and extraction order. Validate: tracker + proposed file map are internally consistent and grounded in current code.
...
- The likely target shape is:
  ...
  - a new shared effect/block layer for reusable editorial beats
  - `src/projects/<id>/` for project-specific assembly, timing, and assets

## Open Questions / Blockers
- What exact folder name should hold the shared higher-level blocks: `src/effects/`, `src/overlay_kit/blocks/`, or another nearby path?
- Which first-wave blocks are stable enough to extract immediately versus still too project-specific?
- Should we add a dedicated “effects lab” composition as the visual showcase, or use an existing project as the primary reference surface?
...
| todo | Inventory the reusable patterns already present in `text-effects`, `c0046`, and `src/overlay_kit/`. | parent |  |
| todo | Propose the shared folder structure and first-wave block list. | parent |  |

## Backlog / Remaining Work
- [ ] Create a concrete extraction map from existing code into shared blocks.
- [ ] Decide and document the shared code location for reusable effect blocks.
- [ ] Extract the first wave of stable blocks from `text-effects` / `c0046`.
- [ ] Create a thin effect catalog doc under `docs/references/`.
- [ ] Decide whether to add an `effects-lab` composition for block previews.
