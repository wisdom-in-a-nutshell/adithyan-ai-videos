...
- 2026-04-09: [IN-PROGRESS] Switched most `C0046` runtime media to remote URLs and uploaded the keyed `S05` matte video, but the live composition still uses the local `subject-keyed-s05-frames/` PNG sequence because it has cleaner binary alpha than the VP9 `.webm`. This means `S05` is still the remaining cloud blocker for the full comp.
