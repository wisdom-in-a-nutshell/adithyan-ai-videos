...
export const TextPlacementDemoOverlay = ({
  ...
  const anchorY =
    variant === 'front' ? 60 : variant === 'behind' ? 44 : 42; // percent
  ...
  const sizeScale = variant === 'fancy' ? 1.35 : 1.12;
  const bigSize = 62 * scale * sizeScale;
  ...
  const textStroke = variant === 'fancy' ? '6px rgba(0,0,0,0.65)' : '5px rgba(0,0,0,0.60)';
  const textShadow =
    variant === 'fancy'
      ? [
  ...
  const color =
    variant === 'fancy' ? 'transparent' : variant === 'front' ? 'rgba(255,255,255,0.98)' : 'rgba(226,232,240,0.92)';
